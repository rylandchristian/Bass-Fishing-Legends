...
1. **`/grill-with-docs`** — sharpen the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grilling` directly — see Standalone. Both paths run the same `/grilling` primitive; `grill-with-docs` is the one that leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
...
- **`/grilling`** — the interview primitive itself: rounds, the frontier, facts are the agent's job and decisions are yours. Also the **stateless** entry (former `/grill-me`): when you are **not working in a working directory**, run `/grilling` directly — it saves nothing locally and builds no `CONTEXT.md`. In a working directory prefer `/grill-with-docs`, which runs the same interview and leaves a paper trail. `/grill-with-docs`, `/triage`, `/wayfinder` and `/improve-codebase-architecture` all run `/grilling` internally.
...
- **`/to-questionnaire`** — when the thing blocking you isn't in your head or the codebase but in **someone else's**, this writes them a questionnaire to fill in. It's the inverse of a `/grilling` interview of you: instead of interviewing you about the subject, it interviews you about the **send** — who it's going to, what you need back — and aims the questions at the gap. What comes back is material for `/grill-with-docs` or `/to-spec`.
